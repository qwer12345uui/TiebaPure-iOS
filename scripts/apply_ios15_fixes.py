...
FILE_RULES = {
    ...
    "TiebaPure/Features/Profile/BrowsingHistoryView.swift": [
        ...
        ("regex",
         ...
         "select-all toolbar item condition"),
        ("regex",
         r"(?m)^([ \t]*)if isEditing == false, historyStore\.items\.isEmpty == false \{\n[ \t]*ToolbarItem\(placement: \.topBarTrailing\) \{",
         r"\1ToolbarItem(placement: .topBarTrailing) {\n\1    if isEditing == false, historyStore.items.isEmpty == false {",
         r"ToolbarItem\(placement: \.topBarTrailing\) \{\s*\n[ \t]*if isEditing == false, historyStore",
         "clear-all toolbar item condition"),
        ("regex",
         r"(?m)^([ \t]*)if visibleEntries\.isEmpty == false \|\| isEditing \{\n[ \t]*ToolbarItem\(placement: \.topBarTrailing\) \{",
         r"\1ToolbarItem(placement: .topBarTrailing) {\n\1    if visibleEntries.isEmpty == false || isEditing {",
         r"ToolbarItem\(placement: \.topBarTrailing\) \{\s*\n[ \t]*if visibleEntries\.isEmpty == false \|\| isEditing \{",
         "edit button toolbar item condition"),
        ("regex",
         r'(?s)(    var body: some View \{\n)[ \t]*(VStack.*?)(\n[ \t]*\.confirmationDialog\(\s*"清空全部浏览历史？",.*?)(\n[ \t]*\.applyingHistoryLifecycleObservers\(\)\n)',
         r'\1        let withNavigation = \2\n        let withDialogs = withNavigation\3\n        return applyingHistoryLifecycleObservers(to: withDialogs)\n',
    ...
    "TiebaPure/Features/Profile/UserProfileView.swift": [
        ...
        ("regex",
         r"(?m)^([ \t]*)if let profile, profile\.isCurrentUser == false \{\n[ \t]*ToolbarItem\(placement: \.topBarTrailing\) \{",
         r"\1ToolbarItem(placement: .topBarTrailing) {\n\1    if let profile, profile.isCurrentUser == false {",
         r"ToolbarItem\(placement: \.topBarTrailing\) \{\s*\n[ \t]*if let profile, profile\.isCurrentUser == false \{",
         "block menu toolbar item condition"),
    ],
